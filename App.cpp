...
void App::addNewProductToDb(string name, double price) {
	DB_PRODUCT[name] = Product(name, price);
	IDS_PRODUCT.push_back(name);
}

void App::addExistingProductToDb(string id, string name, double price) {
	DB_PRODUCT[id] = Product(name, price);
...
time_t App::convertStringTimeFormat2(string strtime) {
	...
	int year, mon, day;
	char c;
	int dumped = sscanf(strtime.c_str(), "%4d%1s%2d%1s%2d", &year, &c, &mon, &c, &day);
	struct tm tm;
	tm.tm_year = year - 1900;
	tm.tm_mon = mon - 1;
	tm.tm_mday = day;
